...
private:
	...
	void erase(int Data, Element*& Root)
	{
		...
		if (Data == Root->Data)
		{
			...
			{
				// для того, чтобы дерево балансировалось при удалении элементов, перед удалением его нужно взвесить
				if (count(Root->pLeft) > count(Root->pRight))
				{
					// и если левая ветка тяжелее чем правая, то берем из нее максмальное значение, потому что оно ближе всего к удаляемому значению 
					Root->Data = maxValue(Root->pLeft);
					erase(maxValue(Root->pLeft), Root->pLeft);
				}
				else
				{
					// в противном случае берем минимальное значение из правой ветки, потому что оно ближе всего к удаляемому значению
...
{
	// int (*function)() - указатель на функцию, которая ничего не прнимает и возращает значение типа int
	clock_t start = clock();
...
void main()
{
	...
	cout << "Среднее-арифетическое элементов дерева: " << tree.avg() << endl;
	...
	cout << "Среднее-арифетическое элементов дерева: " << u_tree.avg() << endl;
